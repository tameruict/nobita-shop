    ...
    <div className="bg-background-dark min-h-screen text-slate-100 font-display flex border-t border-blue-500/20">
      ...
      <main className="flex-1 p-10 overflow-y-auto relative z-10 w-full">
        ...
        <header className="flex justify-between items-center mb-10 relative z-10">
          <div>
            <h1 className="text-3xl font-black tracking-tight mb-2">Product Management</h1>
            <p className="text-slate-400 text-sm">Manage your digital goods and stock.</p>
          </div>
          <button 
            ...
            <div key={p.id} className="glass-panel p-6 rounded-2xl border-slate-800 flex flex-col gap-4 relative group">
              <div className="flex justify-between items-start">
                <div className="flex gap-2">
                  ...
                  <div className={`px-3 py-1 text-[10px] font-bold uppercase rounded border ${p.delivery_type === 'manual' ? 'bg-orange-500/10 text-orange-400 border-orange-500/20' : 'bg-green-500/10 text-green-400 border-green-500/20'}`}>
                    {p.delivery_type === 'manual' ? 'Manual' : 'Auto'}
                  </div>
                </div>
              ...
              <div className="mt-4 flex items-center justify-between text-sm">
                 <div className="flex flex-col">
                    <span className="text-slate-500 text-xs uppercase font-bold tracking-wider">In Stock</span>
             ...
             <div className="glass-panel w-full max-w-md p-8 rounded-3xl border-slate-700 relative animate-in fade-in zoom-in duration-200">
                ...
                <form onSubmit={handleAddProduct} className="space-y-4">
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div>
                        <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Price (VND)</label>
                        <input 
                   ...
                   <div className="flex gap-4 pt-4">
                      <button type="button" onClick={() => setShowAddModal(false)} className="flex-1 py-3 text-slate-400 font-bold hover:text-white transition-colors">Cancel</button>
                      <button type="submit" className="flex-1 bg-primary py-3 rounded-xl font-black text-white shadow-lg shadow-primary/20">Create Product</button>
                   </div>
                </form>
             </div>
          </div>
        ...
        {showImportModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/60 backdrop-blur-sm">
             <div className="glass-panel w-full max-w-lg p-8 rounded-3xl border-slate-700 relative animate-in fade-in zoom-in duration-200">
                <h2 className="text-2xl font-black mb-2">Import Items</h2>
                <p className="text-slate-400 text-sm mb-6">Import accounts/keys for <b>{selectedProduct?.name}</b></p>
                <form onSubmit={handleImportItems} className="space-y-4">
                   <div>
                      <label className="text-xs font-bold text-slate-500 uppercase block mb-1">Contents (acc|pass per line)</label>
                      <textarea 
                        required
                        value={importContent}
                   ...
                   <div className="flex gap-4 pt-4">
                      <button type="button" onClick={() => setShowImportModal(false)} className="flex-1 py-3 text-slate-400 font-bold hover:text-white transition-colors">Cancel</button>
                      <button type="submit" className="flex-1 bg-green-600 py-3 rounded-xl font-black text-white shadow-lg shadow-green-500/20">Confirm Import</button>
                   </div>
                </form>
             </div>
          </div>
